...
class CountryCodesCommunityListTransformSpec extends SpecBase {
  import CountryCodesCommunityListTransformSpec._
  ...
  "transform" - {
    ...
    "when the json matches the expected schema" - {
      ...
      "transforms data as a JsSuccess JsObject and keeps the English description" in {
        ...
        val result = Transformation(CountryCodesCommunityList).runTransform(validData1)
    ...
    "when the json doesn't match the expected schema" - {
      ...
      "returns a JsError parse error if the json doesn't match the expected schema" - {
        ...
        "when missing state" in {
          ...
          val result =
            Transformation(CountryCodesCommunityList).transform
...
object CountryCodesCommunityListTransformSpec {
