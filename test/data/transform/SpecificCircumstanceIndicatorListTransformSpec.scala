...
class SpecificCircumstanceIndicatorListTransformSpec extends SpecBase with ScalaCheckPropertyChecks {
  ...
  "transform" - {
    "when the json matches the expected schema" - {
      "returns the transformed data as a JsSuccess" in {
        ...
        val result = Transformation(SpecificCircumstanceIndicatorList).runTransform(validData)
        ...
        "when top level mandatory fields are missing" in {
          forAll(mandatoryFields) {
            mandatoryField =>
              ...
              val result =
                Transformation(SpecificCircumstanceIndicatorList).transform
                  .reads(dataWithError)
                  .asEither
                  .left
                  .value
